from metamodel.utils.dependencies import get_dependencies
from metamodel.utils.versions import get_versions
...
aceptadas '''
def get_data(file: str, nameWithOwner:str) -> dict[str, list]:
    dependencies = get_dependencies(nameWithOwner, file)
    ...
    for pkg_name in dependencies:
        if dependencies[pkg_name] == 'Any':
            ...
            constraints = get_constraints(parts)
            distributions = get_distributions(pkg_name, constraints)
...
def get_distributions(pkg_name: str, constraints: list[str]) -> list[str]:
    distributions = []
